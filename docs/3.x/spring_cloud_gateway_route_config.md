...
**Args**:
  ...
  - `open_api` (`list[obj]`):  When `null`, the `open_api` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.spring_cloud_gateway_route_config.open_api.new](#fn-spring_cloud_gateway_route_configopen_apinew) constructor.
  - `route` (`list[obj]`):  When `null`, the `route` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.spring_cloud_gateway_route_config.route.new](#fn-spring_cloud_gateway_route_configroutenew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.spring_cloud_gateway_route_config.timeouts.new](#fn-spring_cloud_gateway_route_configtimeoutsnew) constructor.
...
Unlike [azurerm.spring_cloud_gateway_route_config.new](#fn-spring_cloud_gateway_route_confignew), this function will not inject the `resource`
