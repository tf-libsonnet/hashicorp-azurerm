...
**Args**:
  ...
  - `cache` (`list[obj]`):  When `null`, the `cache` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.cdn_frontdoor_route.cache.new](#fn-cdn_frontdoor_routecachenew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.cdn_frontdoor_route.timeouts.new](#fn-cdn_frontdoor_routetimeoutsnew) constructor.
...
Unlike [azurerm.cdn_frontdoor_route.new](#fn-cdn_frontdoor_routenew), this function will not inject the `resource`
