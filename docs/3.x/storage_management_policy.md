...
`azurerm.storage_management_policy.withRule` constructs a mixin object that can be merged into the `storage_management_policy`
...
`azurerm.storage_management_policy.withRuleMixin` constructs a mixin object that can be merged into the `storage_management_policy`
Terraform resource block to set or update the rule field.
