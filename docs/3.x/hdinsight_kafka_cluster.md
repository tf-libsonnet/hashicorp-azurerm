...
`azurerm.list[obj].withRestProxy` constructs a mixin object that can be merged into the `list[obj]`
...
`azurerm.list[obj].withRestProxyMixin` constructs a mixin object that can be merged into the `list[obj]`
Terraform resource block to set or update the rest_proxy field.
