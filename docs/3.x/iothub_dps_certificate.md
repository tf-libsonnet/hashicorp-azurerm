...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.iothub_dps_certificate.timeouts.new](#fn-iothub_dps_certificatetimeoutsnew) constructor.
...
Unlike [azurerm.iothub_dps_certificate.new](#fn-iothub_dps_certificatenew), this function will not inject the `resource`
...
`azurerm.string.withIotDpsName` constructs a mixin object that can be merged into the `string`
