...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.iothub_consumer_group.timeouts.new](#fn-iothub_consumer_grouptimeoutsnew) constructor.
...
Unlike [azurerm.iothub_consumer_group.new](#fn-iothub_consumer_groupnew), this function will not inject the `resource`
...
`azurerm.string.withEventhubEndpointName` constructs a mixin object that can be merged into the `string`
