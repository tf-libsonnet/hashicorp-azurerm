...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.api_connection.timeouts.new](#fn-api_connectiontimeoutsnew) constructor.
...
Unlike [azurerm.api_connection.new](#fn-api_connectionnew), this function will not inject the `resource`
...
`azurerm.string.withManagedApiId` constructs a mixin object that can be merged into the `string`
...
`azurerm.obj.withParameterValues` constructs a mixin object that can be merged into the `obj`
