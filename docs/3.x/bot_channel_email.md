...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.bot_channel_email.timeouts.new](#fn-bot_channel_emailtimeoutsnew) constructor.
...
Unlike [azurerm.bot_channel_email.new](#fn-bot_channel_emailnew), this function will not inject the `resource`
...
`azurerm.string.withEmailAddress` constructs a mixin object that can be merged into the `string`
...
`azurerm.string.withEmailPassword` constructs a mixin object that can be merged into the `string`
