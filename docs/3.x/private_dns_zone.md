...
`azurerm.private_dns_zone.withSoaRecord` constructs a mixin object that can be merged into the `private_dns_zone`
...
`azurerm.private_dns_zone.withSoaRecordMixin` constructs a mixin object that can be merged into the `private_dns_zone`
Terraform resource block to set or update the soa_record field.
