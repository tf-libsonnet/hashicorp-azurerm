...
`azurerm.dns_zone.withSoaRecord` constructs a mixin object that can be merged into the `dns_zone`
...
`azurerm.dns_zone.withSoaRecordMixin` constructs a mixin object that can be merged into the `dns_zone`
Terraform resource block to set or update the soa_record field.
...
`azurerm.dns_zone.withTags` constructs a mixin object that can be merged into the `dns_zone`
