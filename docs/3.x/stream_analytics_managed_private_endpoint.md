...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.stream_analytics_managed_private_endpoint.timeouts.new](#fn-timeoutsnew) constructor.
...
`azurerm.string.withStreamAnalyticsClusterName` constructs a mixin object that can be merged into the `string`
