...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.automation_connection_classic_certificate.timeouts.new](#fn-automation_connection_classic_certificatetimeoutsnew) constructor.
...
Unlike [azurerm.automation_connection_classic_certificate.new](#fn-automation_connection_classic_certificatenew), this function will not inject the `resource`
...
`azurerm.string.withCertificateAssetName` constructs a mixin object that can be merged into the `string`
