...
**Args**:
  ...
  - `name` (`string`): 
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.sentinel_watchlist.timeouts.new](#fn-sentinel_watchlisttimeoutsnew) constructor.
...
Unlike [azurerm.sentinel_watchlist.new](#fn-sentinel_watchlistnew), this function will not inject the `resource`
...
`azurerm.string.withDefaultDuration` constructs a mixin object that can be merged into the `string`
...
`azurerm.string.withItemSearchKey` constructs a mixin object that can be merged into the `string`
