...
**Args**:
  ...
  - `page` (`list[obj]`):  When `null`, the `page` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.bot_channel_facebook.page.new](#fn-bot_channel_facebookpagenew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.bot_channel_facebook.timeouts.new](#fn-bot_channel_facebooktimeoutsnew) constructor.
...
Unlike [azurerm.bot_channel_facebook.new](#fn-bot_channel_facebooknew), this function will not inject the `resource`
