...
**Args**:
  ...
  - `service_id` (`string`): 
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.private_link_service_endpoint_connections.timeouts.new](#fn-timeoutsnew) constructor.
...
Unlike [azurerm.data.private_link_service_endpoint_connections.new](#fn-new), this function will not inject the `data source`
...
`azurerm.string.withServiceId` constructs a mixin object that can be merged into the `string`
