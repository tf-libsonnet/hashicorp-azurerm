...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.image.timeouts.new](#fn-timeoutsnew) constructor.
...
`azurerm.string.withNameRegex` constructs a mixin object that can be merged into the `string`
...
`azurerm.bool.withSortDescending` constructs a mixin object that can be merged into the `bool`
