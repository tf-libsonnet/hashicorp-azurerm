---
permalink: /data/key_vault_secrets/
---

# data.key_vault_secrets

`key_vault_secrets` represents the `azurerm_key_vault_secrets` Terraform data source.
...
**Args**:
  ...
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting data source block. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.key_vault_secrets.timeouts.new](#fn-timeoutsnew) constructor.
...
Unlike [azurerm.data.key_vault_secrets.new](#fn-new), this function will not inject the `data source`
...
**Args**:
  ...
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting object. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.key_vault_secrets.timeouts.new](#fn-timeoutsnew) constructor.
