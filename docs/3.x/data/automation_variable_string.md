---
permalink: /data/automation_variable_string/
---

# data.automation_variable_string

`automation_variable_string` represents the `azurerm_automation_variable_string` Terraform data source.
...
Unlike [azurerm.data.automation_variable_string.new](#fn-new), this function will not inject the `data source`
...
**Args**:
  ...
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting object. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.automation_variable_string.timeouts.new](#fn-timeoutsnew) constructor.
