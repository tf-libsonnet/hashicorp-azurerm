---
permalink: /data/source_control_token/
---

# data.source_control_token

`source_control_token` represents the `azurerm_source_control_token` Terraform data source.
...
**Args**:
  ...
  - `type` (`string`): 
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.source_control_token.timeouts.new](#fn-source_control_tokentimeoutsnew) constructor.
...
Unlike [azurerm.data.source_control_token.new](#fn-source_control_tokennew), this function will not inject the `data source`
