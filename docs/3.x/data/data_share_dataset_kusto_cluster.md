---
permalink: /data/data_share_dataset_kusto_cluster/
---

# data.data_share_dataset_kusto_cluster

`data_share_dataset_kusto_cluster` represents the `azurerm_data_share_dataset_kusto_cluster` Terraform data source.
...
Unlike [azurerm.data.data_share_dataset_kusto_cluster.new](#fn-new), this function will not inject the `data source`
...
**Args**:
  ...
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting object. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.data.data_share_dataset_kusto_cluster.timeouts.new](#fn-timeoutsnew) constructor.
