...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.sentinel_data_connector_office_365.timeouts.new](#fn-sentinel_data_connector_office_365timeoutsnew) constructor.
...
Unlike [azurerm.sentinel_data_connector_office_365.new](#fn-sentinel_data_connector_office_365new), this function will not inject the `resource`
...
`azurerm.bool.withSharepointEnabled` constructs a mixin object that can be merged into the `bool`
