...
**Args**:
  ...
  - `type` (`string`): 
  - `values` (`obj`): 
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [azurerm.automation_connection.timeouts.new](#fn-automation_connectiontimeoutsnew) constructor.
...
Unlike [azurerm.automation_connection.new](#fn-automation_connectionnew), this function will not inject the `resource`
...
`azurerm.obj.withValues` constructs a mixin object that can be merged into the `obj`
