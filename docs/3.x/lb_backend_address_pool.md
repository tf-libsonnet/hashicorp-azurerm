...
`azurerm.lb_backend_address_pool.withTunnelInterface` constructs a mixin object that can be merged into the `lb_backend_address_pool`
...
`azurerm.lb_backend_address_pool.withTunnelInterfaceMixin` constructs a mixin object that can be merged into the `lb_backend_address_pool`
Terraform resource block to set or update the tunnel_interface field.
